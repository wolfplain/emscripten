...
#endif
    if (onsuccess) tableCall(onsuccess, fetch);
    else if (successcb) successcb(fetch);
  };

  var reportProgress = function(fetch, xhr, e) {
    if (onprogress) tableCall(onprogress, fetch);
...
#endif
    if (onerror) tableCall(onerror, fetch);
...
#endif
    if (onreadystatechange) tableCall(onreadystatechange, fetch);
