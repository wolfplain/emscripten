 ...
 ******************************************************************************/
...
#if USE_GLFW == 2
      tableCall(GLFW.active.charFunc, charCode, 1);
#endif

#if USE_GLFW == 3
      tableCall(GLFW.active.charFunc, GLFW.active.id, charCode);
#endif
    },
...
#if USE_GLFW == 2
      tableCall(GLFW.active.keyFunc, key, status);
#endif

#if USE_GLFW == 3
      if (repeat) status = 2; // GLFW_REPEAT
      tableCall(GLFW.active.keyFunc, GLFW.active.id, key, keyCode, status, GLFW.getModBits(GLFW.active));
#endif
    },
...
#if USE_GLFW == 2
      tableCall(GLFW.active.cursorPosFunc, Browser.mouseX, Browser.mouseY);
#endif

#if USE_GLFW == 3
      tableCall(GLFW.active.cursorPosFunc, GLFW.active.id, Browser.mouseX, Browser.mouseY);
#endif
    },
...
#if USE_GLFW == 3
      tableCall(GLFW.active.cursorEnterFunc, GLFW.active.id, 1);
#endif
    },
...
#if USE_GLFW == 3
      tableCall(GLFW.active.cursorEnterFunc, GLFW.active.id, 0);
#endif
    },
...
#if USE_GLFW == 2
      tableCall(GLFW.active.mouseButtonFunc, eventButton, status);
#endif

#if USE_GLFW == 3
      tableCall(GLFW.active.mouseButtonFunc, GLFW.active.id, eventButton, status, GLFW.getModBits(GLFW.active));
#endif
    },
    ...
    onMouseWheel: function(event) {
...
#if USE_GLFW == 2
      tableCall(GLFW.active.scrollFunc, GLFW.wheelPos);
#endif
      ...
      tableCall(GLFW.active.scrollFunc, GLFW.active.id, sx, sy);
#endif
...
#if USE_GLFW == 2
      tableCall(GLFW.active.windowSizeFunc, GLFW.active.width, GLFW.active.height);
#endif

#if USE_GLFW == 3
      tableCall(GLFW.active.windowSizeFunc, GLFW.active.id, GLFW.active.width, GLFW.active.height);
#endif
    },
...
#if USE_GLFW == 3
      tableCall(GLFW.active.framebufferSizeFunc, GLFW.active.id, GLFW.active.width, GLFW.active.height);
#endif
    },
          ...
          if (gamepad) {
            if (!GLFW.joys[joy]) {
              ...
              if (GLFW.joystickFunc) {
                tableCall(GLFW.joystickFunc, joy, 0x00040001); // GLFW_CONNECTED
              }
            }
            ...
            if (GLFW.joys[joy]) {
              ...
              if (GLFW.joystickFunc) {
                tableCall(GLFW.joystickFunc, joy, 0x00040002); // GLFW_DISCONNECTED
              }
    ...
    onDrop: function(event) {
      ...
      function save(file) {
        ...
        reader.onloadend = function(e) {
          ...
          if (++written === count) {
            tableCall(GLFW.active.dropFunc, GLFW.active.id, count, filenames);
    ...
    setWindowSizeCallback: function(winid, cbfun) {
      ...
      tableCall(win.windowSizeFunc, win.width, win.height);
#endif
...
#if USE_GLFW == 2
      tableCall(win.windowSizeFunc, width, height);
#endif

#if USE_GLFW == 3
      tableCall(win.windowSizeFunc, win.id, width, height);
#endif
    },
    ...
    destroyWindow: function(winid) {
      ...
      if (win.windowCloseFunc)
        tableCall(win.windowCloseFunc, win.id);
#endif
